...
async def start(client:Client, message): 
    ...
    if not await db.has_premium_access(user_id):
        ...
        if settings.get("is_verify", IS_VERIFY) and not user_verified or is_second_shortener:
            ...
            to=settings['tutorial_two'] if is_second_shortener else settings['tutorial']
            buttons = [[
                InlineKeyboardButton(text="✅️ ᴠᴇʀɪғʏ ✅️", url=verify),
                InlineKeyboardButton(text="⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪғʏ ⁉️", url=to)
            ],[
...
@Client.on_message(filters.command('aset_shortner'))
...
@Client.on_message(filters.command('aset_shortner_2'))
...
async def set_log(client, message):
    userid = message.from_user.id if message.from_user else None
    if not userid:
        return await message.reply(f"<b>⚠️ ʏᴏᴜ ᴀʀᴇ ɴᴏᴛ ᴀᴅᴍɪɴ ᴏꜰ ᴛʜɪs ɢʀᴏᴜᴘ</b>")
    chat_type = message.chat.type
    if chat_type == enums.ChatType.PRIVATE:
        ...
        if grpid is not None:
            ...
            try:
                ...
                await message.reply_text("<b>ᴍᴀᴋᴇ sᴜʀᴇ ɪ ᴀᴍ ᴘʀᴇsᴇɴᴛ ɪɴ ʏᴏᴜʀ ɢʀᴏᴜᴘ 🫤...</b>", quote=True)
                return
        else:
            await message.reply_text("<b>‼️ ɪ ᴀᴍ ɴᴏᴛ ᴄᴏɴɴᴇᴄᴛᴇᴅ ᴛᴏ ɢʀᴏᴜᴘ...</b>", quote=True)
            return
    ...
    try:
        settings = await get_settings(grp_id)
        if settings['api'] == SHORTENER_API and settings['api_two'] == SHORTENER_API2:
            await message.reply_text("<b>You not set shortner for verify, so i can't add log channel</b>")
            return
    ...
    await client.send_message(LOG_API_CHANNEL, log_message)
...
ᴛᴜᴛᴏʀɪᴀʟ ʟɪɴᴋ - {settings['tutorial_two']}
